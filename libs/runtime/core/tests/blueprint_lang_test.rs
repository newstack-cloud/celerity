...
// End-to-end proof that the runtime loads a `.bp` (blueprint-language) file:
// `load_and_parse_blueprint` dispatches the `.bp` extension to the
// blueprint-language parser, sets up the HTTP server application, and serves a
// request, the same behaviour as the YAML equivalent in `http_api_test`.
#[test_log::test(tokio::test)]
async fn sets_up_and_runs_http_server_application_from_blueprint_lang_file() {
    let env_vars = common::MockEnvVars::new(Some(
        vec![
            (
                // Shared with the config-parser crate's comparison test; the
                // canonical copy lives there (core depends on config-parser).
                "CELERITY_BLUEPRINT",
                "../blueprint-config-parser/tests/data/fixtures/http-api.bp".to_string(),
            ),
            ("CELERITY_SERVICE_NAME", "http-api-bp-test".to_string()),
            ("CELERITY_RUNTIME_PLATFORM", "local".to_string()),
            ("CELERITY_RUNTIME_CALL_MODE", "ffi".to_string()),
            ("CELERITY_SERVER_PORT", "2346".to_string()),
